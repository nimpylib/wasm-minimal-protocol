...
func str_count(s, sub: string): int{.export_typst.} = s.count(sub)
func runeLen(s: string): int{.export_typst.} = unicode.runeLen(s)
...
func withDefaultArgs(a: int, b: int = 42, c: string = "default"): string{.export_typst.} =
  &"a={a}, b={b}, c={c}"
